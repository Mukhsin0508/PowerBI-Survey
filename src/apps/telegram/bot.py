...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    user_language = context.user_data.get(
        'language', "ru"
    ...
    await update.message.reply_text(
        question_text, reply_markup=ReplyKeyboardMarkup(
            [[opt["text"]] for opt in options], one_time_keyboard=True
        ),
    )
    return LANGUAGE
...
async def handle_responses(
    ...
    # ======== District selection ========
        ...
        context.user_data['district_id'] = selected_district[
            'id'
    ...
    if question_id < len(questions)-1:
        ...
        await update.message.reply_text(
            question_text, reply_markup=ReplyKeyboardMarkup(option_texts, one_time_keyboard=True), )
        return next_state
    else:
        # ======== End of questions ========
...
async def income(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    return await handle_responses(
        update, context, question_id=7, next_state=MARITAL_STATUS
    )
